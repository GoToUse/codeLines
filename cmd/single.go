...
var singleLanguageCmd = &cobra.Command{
	Use:   "single",
	Short: "Single means to specify only one language to print their code information",
	Run:   run(),
}
...
func (e *excludeDirArray) Type() string {
	return "excludeDirArray"
}

func init() {
	singleLanguageCmd.Flags().StringVarP(&rootPath, "path", "p", ".", "Root path.")
	singleLanguageCmd.Flags().StringVarP(&suffixName, "suffix", "s", `.go`, "Suffix name of file, starts with `.`; Such as `.go`")
	singleLanguageCmd.Flags().VarP(&excludeDirs, "exclude_folder", "e", "Exclude directories, Multiple -e flags or string seperated by commas or spaces are supported.")
}
